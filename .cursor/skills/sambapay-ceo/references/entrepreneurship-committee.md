...
Local merchants are a second niche, never a client's merchants. In each geography where we have structure, the Website Factory serves the local merchant who wants to digitise in that economy: from the CNPJ through operational management with the local entities, until payout. A client's Brazil store runs on the same website factory. We never take a client's merchants. We never dispute a client's accounts. We only exist because we never compete with our clients.
...
| Character truth | `sambapay-ceo` | Any page that hires a genius of bad character, whatever the technique. Character comes ready on day one or it never will; technique is acquired over a lifetime |
| The obsession test | `sambapay-finance` plus payments committee | Any page where the obsession shrinks to the Website Factory. The obsession stays the relationship with the acquirers, above all, as a category. Never a single brand |

## How this committee writes

1. Name the local niche in its own sentence: a second niche, never a client's merchants.
2. Proof before slogan: the Website Factory works in geographies where we already have structure, on the cheap commercial policy of that geography, because of aggregated volume.
3. Never write "we do not sell to merchants" as if the factory did not exist. The factory is how we serve local merchants.
4. Never write "we answer with the fastest response". Availability is the product, not speed.
5. Never write "taking the client's merchants" as a vector, and never dispute a client's accounts, in any language, to any audience.
...
Do not write the Website Factory as a competing offer. Do not write that a local merchant is a client's merchant. Cut "getting on with" and any integration-language into the acquirer relationship (`payments-marketing-committee`). The local merchant on the Website Factory is another niche, local capture, priced by aggregated volume on the cheap commercial policy of that geography. A client's Brazil store uses the same factory. Two niches. Nobody disputes another's account.
